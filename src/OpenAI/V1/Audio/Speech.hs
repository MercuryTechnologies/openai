...
data CreateSpeech = CreateSpeech
  { model :: Model
  -- ^ Note that only [TTS models](https://platform.openai.com/docs/models#tts) support speech synthesis.
  , input :: Text
  -- ^ The text to generate audio for. The maximum length is 4096 characters.
  , voice :: Voice
  , instructions :: Maybe Text
  -- ^ Instructions for the model to follow when generating the audio.
  -- Does not work with @tts-1@ or @tts-1-hd@ models.
  , response_format :: Maybe Format
  -- ^ Defaults to 'MP3'.
  , speed :: Maybe Double
  -- ^ Defaults to 1.0 (normal speed). Should be between 0.25 and 4.0.
